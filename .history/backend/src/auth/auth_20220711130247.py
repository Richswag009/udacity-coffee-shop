...
AUTH0_DOMAIN = 'richboi.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'Coffee'
...
def get_token_auth_header():
 # Fetch Token from the Authorization Header
   ...
   # validate if the auth header is available
    if not auth_header:
        raise AuthError({
    ...
    # Confrim for invalid token if length == 1
    ...
    # Confirm if the Token is a bearer
   ...
   # Confirm if of type bearer or not
...
def check_permissions(permission, payload):
    # Confirm if permissions array in the JWT
    ...
    # Confirm if the client have permissions to access
